...
アンケート回答（Q1〜Q27の内容）はGASへの保存POSTのみで完結し、回答内容をメールで
通知する処理は行わない（回答内容の正本はresponsesシートのみ）。
アンケート回答と任意の連絡先は別々のPOSTで送信し、個人情報を含まない`response_id`
でのみ関連づける。任意連絡先（Xアカウント・メールアドレス）はresponsesとは別に
leadsへ保存され、連絡希望があったことだけをFormSubmitへベストエフォートで通知する
（失敗しても連絡先の保存状態・GA4計測には一切影響しない）。再訪リードの`response_id`は、
Cookie/localStorageから復元できる値をクライアントが「申告」するだけで、実際に
responsesシートに存在するかはGAS側が確認してから`link_status`（`linked` / `unlinked`）
を決める。

内部トリアージ用スコア・判定は、連絡先フォーム送信時の通知データ（メール本文）にのみ
含め、回答者の画面やGA4には一切表示・送信しない。連絡先フォーム送信時（+3点）は
