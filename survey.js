 ...
 *
 * 【PR #108レビュー対応】Q4/Q6（緊縛への関心・立場）がIssue #107で全ジェンダー共通設問へ
 * 移動したことに伴い、computeScore()は性自認が男性の回答にのみスコアを算出する
 * （男性以外はnullを返し、内部スコア・内部判定を送信メールへ含めない）。このスコアは
 * 「男性のユニフォーム×緊縛企画への見込み度」を測るためのものであり、女性・その他は
 * Q7以降の男性向け設問（スポーツ・ジム・体格・来訪可能性・参加意向等）に一切回答しないため、
 * Q4/Q6だけの加点が乗ったスコア・ランクは本来の意味を持たない。
 * ─────────────────────────────────────────────────────────────
 */
window.__Survey = {};
...
(function () {
  ...
  /* gas/ataru_survey_public/ をWebアプリとしてデプロイしたURL。
     ...
     （Issue #104 追加指示5）。 */
  var GAS_ENDPOINT = 'https://script.google.com/macros/s/AKfycbw0l3yLTjJEjIo0KBpP4LarkpMKyGd_7zbcEgXEcNmyCZuNgz5F-iYvfZqMThARnBf3KQ/exec';
  ...
  var TXT = {
    /* Q4自体は必須（分岐条件・全体概要に使うため）。「ご回答は任意です」と案内すると
       「回答しない」選択肢のないQ4で先へ進めなくなり画面と矛盾するため、
       任意なのはQ5・Q6・Q6-Aである旨を明示する（PR #108レビュー指摘対応）。
       「Q5以降」という表現だと、実際にはQ5より後にもQ15/Q17/Q18/Q22/Q24/Q25等の
       必須設問があるため誤解を招く。全ジェンダー共通ブロック内で必須はQ4のみである
       ことが分かるよう、任意な設問をQ5・Q6・Q6-Aと個別に列挙する。 */
    bondageIntro: 'ここから、緊縛・ロープ表現への関心についてお聞きします。Q5・Q6・Q6-Aの詳細項目は任意です。答えたくない項目は「回答しない」等の選択肢がある場合はそちらを選べます。',
...
(function () {
  ...
  /* 内部トリアージ用スコア。新Q番号の保存キーのみを参照する（旧保存キーは参照しない）。
     ...
     このスコアは「男性のユニフォーム×緊縛企画への見込み度」を測るためのものであり、
     加点項目（スポーツ・ジム・体格・野球ユニ・来訪可能性・参加意向等）は男性のみに
     表示されるQ7以降の設問が前提になっている。Q4/Q6（緊縛への関心・立場）はIssue #107で
     全ジェンダー共通設問へ移動したため、女性・その他でも該当項目に回答すればここへ
     加点されてしまう。しかし女性・その他はQ7以降のスポーツ/ユニフォーム/来訪/参加意向を
     一切回答しないため、それらの加点だけが乗った状態のスコア・ランクは「男性ユニ見込み層」
     という本来の意味を持たず、内部トリアージを誤らせる。そのため、性自認が男性でない
     回答にはスコア自体を算出しない（nullを返す）。 */
  function computeScore(a) {
    if (a.q2_gender !== '男性') return null;
    var score = 0;
  ...
  function rankFromScore(score) {
    /* computeScore()が男性以外にnullを返すため、ここでもnull/undefinedを
       そのまま「一般回答」等へ丸めず、ランクなし（null）として扱う。 */
