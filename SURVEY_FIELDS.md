...
アンケート回答自体の送信（`realSubmit()`）はGASへの保存POSTのみで、スコア計算は
一切行わない・回答内容をメール通知することもない。スコア計算は任意連絡先フォーム
（リード送信）でのみ行われ、そこで例外が発生しても `safeComputeScore` /
`safeRankFromScore` を通すことで連絡先の送信処理を継続する（スコア関連フィールドを
省略して送信する）。

**性自認が男性の回答にのみスコアを算出する**（PR #108レビュー対応）。
`computeScore()` は `a.q2_gender !== '男性'` の場合 `null` を返し、
`rankFromScore(null)` も `null` を返す。連絡先フォームはスコアが `null` のとき
「内部スコア」「内部判定」欄を通知データへ含めない。

このスコアは「男性のユニフォーム×緊縛企画への見込み度」を測るためのもので、
Q7以降の男性のみ表示される設問（スポーツ経験・ジム頻度・体格・名古屋来訪可能性・
参加意向等）を前提に加点している。Issue #107でQ4（緊縛への関心）・Q6（立場）が
全ジェンダー共通設問へ移動したため、性自認によるガードがないと、Q7以降に一切
回答しない女性・その他の回答にもQ4/Q6由来の加点だけが乗ってしまい、
「男性ユニ見込み層」という本来の意味を持たないスコア・ランクが生成されてしまう
（例: 女性が「25〜29歳」「緊縛に興味がある」「縛られる側」を選び連絡先を送ると、
性自認ガードがない場合は加点だけで「育成候補」判定になり得た）。
